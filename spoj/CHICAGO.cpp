...
int main()
{
   ...
   while(1)
   {
   		...
   		REP(i,n)	DIS[i] = 0.0;
   		int x,y,c;
   		REP(i,m)	cin >> x >> y >> c , v[x-1].PB(MP(y-1,(double)c/100.0)) , v[y-1].PB(MP(x-1,(double)c/100.0));
		set <pair < double , int > > myset;
		myset.insert(MP(1.0,0));
		...
		while(!myset.empty())
		{
			...
			REP(j,si)
				...
				if(DIS[te4]*v[te4][j].second > DIS[v[te4][j].first] )	
					{
					if(myset.find(MP(DIS[v[te4][j].first],v[te4][j].first))!=myset.end())
						myset.erase(MP(DIS[v[te4][j].first],v[te4][j].first));
					DIS[v[te4][j].first]=DIS[te4]*v[te4][j].second;
					myset.insert(MP(DIS[v[te4][j].first],v[te4][j].first));
					}
				}
		}
   		REP(i,n)	v[i].clear();
